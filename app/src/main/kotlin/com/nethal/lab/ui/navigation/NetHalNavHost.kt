...
private object Routes {
    ...
    // spec (Tela 3 → Tela 4 → Tela 5) — decisão confirmada (ver KDoc de PairingAuthViewModel):
    // ler capabilities exige sessão autenticada nesta implementação, então autenticar vem antes.
    // A própria rota "authentication" virou o cluster `PairingAuthRoutes.GRAPH`
    // (`:feature:pairing-auth`, issues #76-#79) — sem constante própria aqui.
...
fun NetHalNavHost(
    ...
    // navegação entre o grafo de pareamento por descoberta (`pairingDiscoveryGraph`) → grafo de
    // pareamento por autenticação (`pairingAuthGraph`) → "capabilities" → "report". Os
    // ViewModels dessas telas precisam desses valores no construtor (não dá para injetar via
    // `SavedStateHandle` sem Parcelable/serializer dedicado nesta entrega) — factory/parâmetro
    ...
    // Sessão autenticada entregue pelo grafo de autenticação
    // (`PairingAuthViewModel.captureAuthenticatedSession`) para a Tela 4 reaproveitar sem
    // autenticar de novo — mesmo padrão de estado compartilhado usado acima para `NetworkTarget`.
    // `null` até o grafo de autenticação confirmar sucesso.
    ...
    NavHost(navController = navController, startDestination = Routes.WELCOME) {
        ...
        // Telas 2c/2d/2e/2f (issues #76, #77, #78, #79) — módulo :feature:pairing-auth.
        pairingAuthGraph(
            navController = navController,
