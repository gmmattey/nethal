...
- Ao abrir ou triar issue, seguir a skill global `/issue-conventions`: título `Task - <descrição>` para trabalho planejado e `[BUG] <descrição>` para defeito, no GitHub Issues (`gmmattey/signallq-nethal`), com label de tipo/prioridade quando fizer sentido.
...
4. Crio a issue: `gh issue create --repo gmmattey/signallq-nethal --title "..." --body-file <arquivo> --label "<tipo>" --label "<prioridade>"`
...
**Consultas laterais permitidas:** antes de criar a issue, verifico se issue similar já existe (`gh issue list --repo gmmattey/signallq-nethal --search "<termo>"`).
