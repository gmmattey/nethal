...
- **Abrir bug**: no GitHub Issues (`7AgentsStudio/nethal`) no formato `[BUG]` conforme `/issue-conventions`.
...
```
gh pr view <N> --repo 7AgentsStudio/nethal --json state,merged,mergedAt,mergeCommit
```
...
- Arquivos tocados: `gh pr diff <N> --repo 7AgentsStudio/nethal --name-only`
- Diff completo: `gh pr diff <N> --repo 7AgentsStudio/nethal`
...
**O que faço:**
1. Leio a issue: `gh issue view N --repo 7AgentsStudio/nethal`
2. Reviso o código da PR via GitHub, nunca via estado local: `gh pr diff <N> --repo 7AgentsStudio/nethal --name-only` primeiro, depois o conteúdo
